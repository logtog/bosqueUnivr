<!doctype html>
<html>
	<head>
		<title>Projeto HAR</title>
		<link href="css/qrcode.css" type="text/css" rel="stylesheet" href="https://unicons.iconscout.com/release/v4.0.0/css/solid.css"/>
		<script type="text/javascript" src="js/qrcode.js"></script>
		<link href="https://fonts.googleapis.com/css2?family=Saira+Extra+Condensed:wght@400;500;700&display=swap" rel="stylesheet">
		<link href="https://fonts.googleapis.com/css2?family=Nunito:wght@300&display=swap" rel="stylesheet">
		<meta charset="utf-8" />
	</head>

	<!----------------------------------HEADER---------------------------------------->

	<body>
		<div class="header">
			<div class="center">
				<img src="images/UNIVR.png" width="145px" height="50px" onclick="main()" />
			</div><!--center-->					   
		</div><!--header-->
		<div class="bg-onda">	
			<img class="onda1" src="images/svg/ondas.svg" width="280%" height="362px"/>
		</div><!--waves-->	

	<!----------------------------------SECTION DAS ESPÉCIES-------------------------->

		<div class="section-1">
			<div class="center-text">
				<div class="titulo">
					<h2>Espécies Nativas </h2>	
				</div>	

				<div class="container-section" onclick="especie()">
					<div class="container-single">
						<div><img src="images/svg/capture.svg" width="80px" height="80px"/></div>
							<div class="text-container-single">
								<h2>Espécie 1</h2>
							</div><!--text-container-single-->					
					</div><!--container--single-->

					<div class="container-single">
						<div><img src="images/svg/capture.svg" width="80px" height="80px"/></div>
						<div class="text-container-single">
							<h2>Espécie 2</h2>
						</div><!--text-container-single-->					
					</div><!--container--single-->


					<div class="container-single">
						<div><img src="images/svg/capture.svg" width="80px" height="80px"/></div>
						<div class="text-container-single">
							<h2>Espécie 3</h2>
						</div><!--text-container-single-->					
					</div><!--container--single-->


					<div class="container-single">
						<div><img src="images/svg/capture.svg" width="80px" height="80px"/></div>
							<div class="text-container-single">
								<h2>Espécie 4</h2>
							</div><!--text-container-single-->					
					</div><!--container--single-->


					<div class="container-single">
						<div><img src="images/svg/capture.svg" width="80px" height="80px"/></div>
						<div class="text-container-single">
							<h2>Espécie 5</h2>
						</div><!--text-container-single-->					
					</div><!--container--single-->


					<div class="container-single">
						<div><img src="images/svg/capture.svg" width="80px" height="80px"/></div>
						<div class="text-container-single">
							<h2>Espécie 6</h2>
						</div><!--text-container-single-->					
					</div><!--container--single-->

					<div class="container-single">
						<div><img src="images/svg/capture.svg" width="80px" height="80px"/></div>
							<div class="text-container-single">
								<h2>Espécie 7</h2>
							</div><!--text-container-single-->					
					</div><!--container--single-->


					<div class="container-single">
						<div><img src="images/svg/capture.svg" width="80px" height="80px"/></div>
						<div class="text-container-single">
							<h2>Espécie 8</h2>
						</div><!--text-container-single-->					
					</div><!--container--single-->


					<div class="container-single">
						<div><img src="images/svg/capture.svg" width="80px" height="80px"/></div>
						<div class="text-container-single">
							<h2>Espécie 9</h2>
						</div><!--text-container-single-->					
					</div><!--container--single-->

					<div class="container-single">
						<div><img src="images/svg/capture.svg" width="80px" height="80px"/></div>
							<div class="text-container-single">
								<h2>Espécie 10</h2>
							</div><!--text-container-single-->					
					</div><!--container--single-->


					<div class="container-single">
						<div><img src="images/svg/capture.svg" width="80px" height="80px"/></div>
						<div class="text-container-single">
							<h2>Espécie 11</h2>
						</div><!--text-container-single-->					
					</div><!--container--single-->


					<div class="container-single">
						<div><img src="images/svg/capture.svg" width="80px" height="80px"/></div>
						<div class="text-container-single">
							<h2>Espécie 12</h2>
						</div><!--text-container-single-->					
					</div><!--container--single-->


					<div class="clear"></div>
				</div><!--container-section-->
			</div><!--center-->
		</div><!--section-1-->

		<div class="footer">
			<a href="https://portal.unisepe.com.br/univr/">Bairro: Jardim das Palmeiras Rua: Oscar Yoshiaki Magário, nº 185. Registro - São Paulo</a>
		</div><!--footer-->
	</body>
</html>